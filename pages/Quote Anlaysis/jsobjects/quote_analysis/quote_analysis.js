export default {
		...
		var chart = {
			...
			"dataSource": {
				"chart": {
					...
					"xAxisMinValue": "0.00",
					"xAxisMaxValue": "1.00",
					"palettecolors": "042A2B,850101,5EB1BF,CDEDF6,EF7B45,D84727"
				}
			}
		};
	...
	chartflood100risk: async () => {
		
		var chart = {
			"type": "scatter",
			"dataSource": {
				"chart": {
					"caption": "Flood Depth 100 Risk Analysis",
					"xAxisName": "Sum Insured",
					"yAxisName": "Flood Depth",
					"theme": "fusion",
					"alignCaptionWithCanvas": 1,
					"captionFontSize": "24",
					"captionAlignment": "center",
		...
		await Flood_100_Per_TSI.run({quote_id: appsmith.store.quote_id});
		
		var data = [];
		Flood_100_Per_TSI.data.forEach( r => {
			data.push( { x: ' '+ r.tsi, y: r.flood, tooltext: r.tooltext +"{br}" + r.flood });
		});
		var dataset = [];
		var series = { seriesname: "Flood Risk", data};
		dataset[0] = series;
		chart.dataSource.dataset = dataset;
		await storeValue('chart_flood100risk', chart, false);
		ChartFloodDepth100.isVisible = true;
	},
